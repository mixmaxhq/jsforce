 ...
 * @param {Callback.<Request>} [logger] - Callback function to log response errors.
 * @returns {Promise.<UserInfo>}
 */
Connection.prototype.authorize = function(code, callback, externalLogger) {
  var self = this;
  var logger = this._logger;

  return this.oauth2.requestToken(code, null, externalLogger).then(function(res) {
    var userInfo = parseIdUrl(res.id);
    self.initialize({
      instanceUrl : res.instance_url,
      accessToken : res.access_token,
